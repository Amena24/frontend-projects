...
if(sessionStorage.getItem("isLoggedIn") !== "true"){
   ...
   if (h === 0 && m === 0 && s === 0) {
      clearInterval(timer);
      window.close()
    var child = window.open("timeout.html")
...
//    ////////////////////////////////////////////////////////////////////////////////////////
    ...
    for (var j = 0; j < 10; j++) {
     ...
     newarray.forEach(function(element,index){
      element.querySelector("p span").innerHTML +=`${index+1}. `
      localStorage.setItem(`q${index}`,element.outerHTML)
      console.log(element)
...
function saveAnswer(){
 ...
 newarray.forEach((question,i)=>{
  selected = question.querySelector('input[type="radio"]:checked')
  
           if(selected)
            answer[i]=(selected.value)
          else
             answer[i]=null
